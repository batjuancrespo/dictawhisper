...
function correctSelectedText() {
    ...
    // Añadir al diccionario personalizado si son palabras simples
    if (selectedText.split(/\s+/).length === 1 && correctedText.split(/\s+/).length === 1) {
        ...
        if (learningEnabled) {
            ...
            console.log(`Nueva corrección compleja registrada: "${selectedText}" → "${correctedText}"`);
            addToDictionary(selectedText, correctedText);
            console.log(`También añadida al diccionario como frase completa`);
            
            pendingSyncChanges = true;
            updateSyncStatus("Cambios pendientes...");
        }
        
        showTempMessage("Texto corregido y guardado");
    }
...
function applyCustomDictionary(text) {
    ...
    customDictionary.forEach(entry => {
        // Crear expresión regular con límites de palabra
...
function startDictation() {
    ...
    try {
        // Verificar si ya está escuchando para evitar errores
        if (!isListening) {
            recognition.start();
            isListening = true;
        } else {
...
async function loadDictionary() {
    try {
        ...
        if (docSnap.exists() && docSnap.data().dictionary) {
            ...
            customDictionary.forEach(entry => {
                console.log(`- Entrada: "${entry.commonMistake}" → "${entry.correctWord}"`);
            });
        } else {
            console.log("No hay diccionario guardado en Firebase para este usuario, inicializando vacío");
            customDictionary = [];
            
            // Añadir corrección por defecto para "consecuencias potenciadas"
