...
let startRecordBtn, pauseResumeBtn, retryProcessBtn, copyPolishedTextBtn, correctTextSelectionBtn, 
    statusDiv, polished user.uid : null); 
        if (user) {
            ...
            userDisplaySpan.textContent = `${user.email || 'Usuario'}`;Textarea, audioPlayback, audioPlaybackSection, 
...
let recording
            ...
            if (!window.dictationAppInitialized) {
                ...
                if (typeof updateButtonStates === "function")Seconds = 0;  
...
let commonMistakeNormalization updateButtonStates("initial"); 
            }
        } else {
            currentUserId = null; 
            customVocabulary = {}; 
            ...
            authContainer.style.display = 'block'; appContainer.style. = {}; // Corresponderá a normalizations
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    startRecordBtn = document.getElementById('startRecordBtndisplay = 'none';
    ...
    themeSwitch = document.getElementById(': initializeAuthAndApp - onAuthStateChanged listener suscrito.");
    ...
    modalAddNewRuleButtonVocab = document.getElementById('modalAddNew}
} 
...
function initializeDictationAppLogic(userId) {
    ...
    if (!allElementsFound) {
        const errorMessage = "Error crítico: Uno o más elementos HTML de la app no se encontraron al cargar el DOMPauseResume();
            setTimeout(() => { isProcessingClick = false; }, CLICK_DEBOUNCE_MS);
        }); 
    ...
    setAccentRGB(); 
    new MutationObserver(setAccentRGB).observe(document.body, { attributes: true, attributeFilter: ['data-{await navigator.clipboard.writeText(t); setStatus("¡Texto copiado!", "success", 2000);}catch(e){console.error('Error copia:',e);setStatus("Error copia.", "error", 3000);}}); copyPolishedTextBtn.dataset.listenerAttached = 'true';}
    ...
    if (clearHeaderButton && !clearHeaderButton.theme']});
...
function initializeAuthAndApp() {
    // ... (Lógica de autenticación como en la respuesta anterior completa)...
...
dataset.listenerAttached) { clearHeaderButton.addEventListener('click', () => { headerArea.value = ""; headerArea.focus(); }); clearHeaderButton.dataset.listenerAttached = 'true';}
    ...
    if (!document.body.dataset.keydownListenerAttached) { 
        document.addEventListener('keydown', function(event) {
            if (event.shiftKey && (event.metaKey || event.ctrlKey    const signupErrorDiv = document.getElementById('signup-error');
    ...
    showLoginLink.addEventListener('click', (e) =>) && event.key === 'Shift') {
...
function setAccentRGB() { try { const bS = getComputedStyle(document.body); if (!bS) return; const aC = bS.getPropertyValue('--accent-color').trim(); if (aC.startsWith('#')) { const r = parseInt(aC.slice(1,3), { e.preventDefault(); signupForm.style.display = 'none'; loginForm.style.display = 'block'; loginErrorDiv.textContent = ''; signupErrorDiv.textContent = ''; });
    ...
    logoutButton.addEventListener('click', async () => { try { await signOut(auth); } catch (error) { console.error('DEBUG: Error al cerrar sesión:', error); alert("Error al cerrar sesión."); } });
    onAuthStateChanged(auth, async (user) => { if (user) { currentUserId = user.uid; document.body.classList.remove('logged-out'); document.body.classList.add('logged-in'); authContainer.style.display = 'none'; appContainer.style.display = 'flex16), g = parseInt(aC.slice(3,5),16), b = parseInt(aC.slice(5,7),16); document.documentElement.style.setProperty('--accent-color-rgb',`${r},${g},${b}`); } else if (aC.startsWith('rgb')) { const p = aC.match(/[\d.]+/g); if (p && p.length >=3) document.documentElement.style.setProperty('--accent-color-rgb',`${p[0]},${p[1]},${p[2]}`);}} catch (e) { console.warn("Failed to set --accent-color-rgb:", e); }}
...
function setupVolumeMeter(stream) { volumeMeterContainer.style.display='block'; if(!audioContext) audioContext=new(window.AudioContext||window.webkitAudioContext)(); if(audioContext.state=='; userDisplaySpan.textContent = `${user.email || 'Usuario'}`; await loadUserVocabularyFromFirestore(currentUserId); if (!window.dictationAppInitialized) { initializeDictationAppLogic(currentUserId); window.dictationAppInitialized = true; } else { if (typeof updateButtonStates === "function") updateButtonStates("initial"); } } else { currentUserId = null; customVocabulary = {}; learnedCorrections = {}; commonMistakeNormalization = {}; document.body.classList.remove('logged-in'); document.body.classList.add('logged-out'); authContainer.style.display = 'block'; appContainer.style.display = 'none'; if (userDisplaySpan) userDisplaySpan.textContent = ''; if (window.currentMediaRecorder && window.currentMediaRecorder.state !== "inactive") { try { window.currentMediaRecorder.stop(); } catch(e) { console.warn("DEBUG: onAuthStateChanged - Error al detener MediaRecorder en logout:", e); } } window.dictationAppInitialized = false; } });
    function getFirebaseErrorMessage(error) { switch (error.code) { case 'auth/invalid-email': return 'Email inválido.'; case 'auth/user-disabled': return 'Usuario deshabilitado.'; case 'auth/user-not-found': return 'Usuario no encontrado.'; case 'auth/wrong-password': return 'Contraseña incorrecta.'; case 'auth/email-already-in-use': return 'Email ya registrado.'; case 'auth/weak-password': return 'Contraseña débil.'; default: return error.message || "Error auth."; } }
} 

function initializeDictationAppLogic(userId) {
    console.log(`DEBUG: initializeDictationAppLogic para usuario: ${userId} - Asignando listeners.`);
    if (!startRecordBtn.dataset.listenerAttached) { startRecordBtn='suspended') audioContext.resume(); analyser=audioContext.createAnalyser(); microphoneSource=audioContext.createMediaStreamSource(stream); microphoneSource.connect(analyser); analyser.fftSize=256; analyser.smoothingTimeConstant=0.3; const l=analyser.frequencyBinCount, d=new Uint8Array(l); function draw(){if(!isRecording||isPaused){if(isPaused){volumeMeterBar.classList.add('paused'); volumeMeterBar.style.background='var(--button-default-bg)';}else{volumeMeterBar.classList.remove('paused'); volumeMeterBar.style.background='var(--volume-bar-gradient)';} animationFrameId=requestAnimationFrame(draw); return;} animationFrameId=requestAnimationFrame(draw); analyser.getByteFrequencyData(d); let s=0; for(let i=0;i<l;i++){s+=d[i];} let a=s/l; let v=(a/130)*100; v=Math.min(100,Math.max(0,v)); volumeMeterBar.style.width=v+'%'; volumeMeterBar.classList.remove('paused'); volumeMeterBar.style.background='var(--volume-bar-gradient)';} draw(); }
...
async function startActualRecording() { 
    console.log("DEBUG startActualRecording: polishedTextarea.selectionStart =", polishedTextarea.selectionStart, "polished.addEventListener('click', () => { if (isProcessingClick) return; isProcessingClick = true; toggleRecordingState(); setTimeout(() => { isProcessingClick = false; }, CLICK_DEBOUNCE_MS); }); startRecordBtn.dataset.listenerAttached = 'true';}
    ...
    if (correctTextSelectionBtn && !correctTextSelectionBtn.dataset.listenerAttached) { correctTextSelectionBtn.addEventListener('click', handleCorrectTextSelection); correctTextarea.selectionEnd =", polishedTextarea.selectionEnd);
    ...
    try {
        ...
        mediaRecorder.onresume = () => { setStatus(isDictatingForReplacement ? 'Dictando reemplazo...'TextSelectionBtn.dataset.listenerAttached = 'true';}
    if (techniqueButtonsContainer && !techniqueButtonsContainer.dataset.listenerAttached) { techniqueButtonsContainer.addEventListener('click', (e) => { if (e.target.tagName === 'BUTTON' && e.target.dataset.techniqueText) { headerArea.value = e.target.dataset.techniqueText; headerArea.focus(); }}); techniqueButtonsContainer.dataset.listenerAttached = 'true';}
    ...
    if (modalAddNewRuleButtonVocab && !modalAddNewRuleButtonVocab.dataset.listenerAttached) { modalAddNewRuleButtonVocab.addEventListener('click', handleAddNewVocabRule); modalAddNewRuleButtonVocab.dataset.listenerAttached = 'true'; }
    if (vocabManagerModal && !vocabManagerModal.dataset.listenerAttached) { vocabManagerModal.addEventListener('click', (e) => { if (e.target === vocabManagerModal) closeVocabManager(); }); vocabManagerModal.dataset.listenerAttached = 'true'; }
    if (!document.body.dataset.keydownListenerAttached) { document.addEventListener('keydown', function(event) { if (event.shiftKey && (event.metaKey || event.ctrlKey) && event.key === 'Shift') { event.preventDefault(); if (document.body.classList.contains('logged-in') && startRecordBtn && !startRecordBtn.disabled) { if (isProcessingClick) return; isProcessingClick = true; toggleRecordingState(); setTimeout(() => { isProcessingClick = false; }, CLICK_DEBOUNCE_MS); } } else if (event.shiftKey && event.altKey && event.key.toUpperCase() === 'P') { event.preventDefault(); if (document : 'Grabando... (Reanudado)', 'processing'); isPaused = false; volumeMeterBar.classList.remove('paused'); volumeMeterBar.style.background = 'var(--volume-bar-gradient)'; updateButtonStates("recording"); };
...
async function processAudioBlobAndInsertText(audioBlob) {
    ...
    try {
        const.body.classList.contains('logged-in') && pauseResumeBtn && !pauseResumeBtn.disabled) { if (isProcessingClick) return; isProcessingClick = true; handlePauseResume(); setTimeout(() => { isProcessingClick = false; }, CLICK_DEBOUNCE_MS); } } }); document.body.dataset.keydownListenerAttached = 'true'; }
    updateButtonStates("initial"); 
} 

function applyTheme(theme) { document.body.setAttribute('data-theme', theme); localStorage.setItem('theme', theme); if (themeSwitch) themeSwitch.checked = theme === 'dark'; if (mainTitleImage && mainTitleImageDark) { mainTitleImage.style.display = theme === 'light' ? 'inline-block' : 'none'; mainTitleImageDark.style.display = theme === 'dark' ? 'inline-block' : 'none'; } }
function setAccentRGB() { try { const bS = getComputedStyle(document.body); if (!bS) return; const aC = bS.getPropertyValue('--accent-color').trim(); if (aC.startsWith('#')) { const r = parseInt(aC.slice(1,3), base64Audio = await blobToBase64(audioBlob);
        ...
        if (isDictatingForReplacement) {
            ...
            if (textBeforeSelection.length > 0 && !/\s$/.16), g = parseInt(aC.slice(3,5),16), b = parseInt(aC.slice(5,7),16); document.documentElement.style.setProperty('--accent-color-rgb',`${r},${g},${b}`); } else if (aC.startsWith('rgb')) { const p = aC.match(/[\d.]+/g); if (p && p.length >=3) document.documentElement.style.setProperty('--accent-color-rgb',`${p[0]},${p[1]},${p[2]}`);}} catch (e) { console.warn("Failed to set --accent-color-rgb:", e); }}
function setStatus(message, type = "idle", duration = 0) { if (!statusDiv) return; statusDiv.textContent = message; statusDiv.className = ''; statusDiv.classList.add(`status-${type}`); if (duration > 0) { setTimeout(() => { if (statusDiv.textContent === message) updateButtonStates("initial"); }, duration); }}
function startRecordingTimer() { stopRecordingTimer(); updateRecordingTimeDisplay(); recordingTimerInterval = setInterval(() => { if (!isPaused) { recordingSeconds++; updateRecordingTimeDisplay();}}, 1000); }
function stopRecordingTimer() { clearInterval(recordingTimerInterval); }
function updateRecordingTimeDisplay() { const m=Math.floor(recordingSeconds/60), s=recordingSeconds%60; recordingTimeDisplay.textContent = isRecording||isPaused ? `Tiempo: ${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}` : ""; }
function resetRecordingTimerDisplay() { recordingTimeDisplay.textContent = ""; recordingSeconds = 0; }
function setupVolumeMeter(stream) { volumeMeterContainer.style.display='block'; if(!audioContext) audioContext=new(window.AudioContext||window.webkitAudioContext)(); if(audioContext.state==='suspended') audioContext.resume(); analyser=audioContext.createAnalyser(); microphoneSource=audioContext.createMediaStreamSource(stream); microphoneSource.connect(analyser); analyser.fftSize=256; analyser.smoothingTimeConstant=0.3; const l=analyser.frequencyBinCount, d=new Uint8Array(l); function draw(){if(!isRecording||isPaused){if(isPaused){volumeMeterBar.classList.add('paused'); volumeMeterBar.style.background='var(--button-default-bg)';}else{volumeMeterBar.classList.remove('paused'); volumeMeterBar.style.background='var(--volume-bar-gradient)';} animationFrameId=requestAnimationFrame(draw); return;} animationFrameId=requestAnimationFrame(draw); analyser.getByteFrequencyData(d); let s=0; for(let i=0;i<l;i++){s+=d[i];} let a=s/l; let v=(a/130)*100; v=Math.min(100,Math.max(0,v)); volumeMeterBar.style.width=v+'%'; volumeMeterBar.classList.remove('paused'); volumeMeterBar.test(textBeforeSelection) && processedNewText.length > 0 && !/^\s/.test(processedNewText) && !/^[,.;:!?)]/.test(processedNewText)) { smartSpaceBefore = " "; }
            let smartSpaceAfter = "";
...
function handlePauseResume() { if(!mediaRecorder||!isRecording)return;if(mediaRecorder.state==="recording"){mediaRecorder.pause();}else if(mediaRecorder.state==="paused"){mediaRecorder.resume();}}style.background='var(--volume-bar-gradient)';} draw(); }
function stopVolumeMeter() { if(animationFrameId) cancelAnimationFrame(animationFrameId); if(microphoneSource){microphoneSource.disconnect(); microphoneSource=null;} volumeMeterBar.style.width='0%'; volumeMeterBar.classList.remove('paused'); volumeMeterContainer.style.display='none';}
function toggleRecordingState() { if(isRecording){if(mediaRecorder&&(mediaRecorder.state==="recording"||mediaRecorder.state==="paused")){mediaRecorder.stop();setStatus("Deteniendo...","processing");}else{isRecording=false;isPaused=false;updateButtonStates("initial");}}else{startActualRecording();}}
async function startActualRecording() { 
    if (polishedTextarea.selectionStart !== polishedTextarea.selectionEnd) {
...
async function processAudioBlobAndInsertText(audioBlob) {
    ...
    try {
        ...
        if (isDictatingForReplacement) {
            ...
            // Capitalización contextual para el texto reemplazado
            if (processedNewText.length > 0) {
                const charBefore = textBeforeSelection.trim().slice(-1); 
                const needsCapital = replacementSelectionStart === 0 || charBefore === '.' || charBefore === '!' || charBefore === '?' || textBeforeSelection.trim().endsWith('\n');
                console.log(`DEBUG processAudioBlobAndInsertText (Reemplazo): Char antes: '${charBefore}', ¿Necesita Mayúscula?: ${needsCapital}`);
...
async function callGeminiAPI(p,isTxt=false){if(!userApiKey)throw new Error('No API Key');const u=`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${userApiKey}`;const t=isTxt?0.1:0.2;const y={contents:[{parts:p}],generationConfig:{temperature:t}};console.log(`Gemini (isTxt:${isTxt},temp:${t}). Prompt(inicio):`,JSON.stringify(p[0]).substring(0,200)+"...");const resp=await fetch(u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(y)});if(!resp.ok){const eD=await resp.json();console.error("Error Gemini API:",eD);throw new Error(`Error API:${eD.error?.message||resp.statusText}(${resp.status})`);}const d=await resp.json();if(d.candidates?.[0]?.content?.parts?.[0]?.text)return d.candidates[0].content.parts[0].text;if(d.promptFeedback?.blockReason)throw new Error podría necesitar más inteligencia.
            ...
            console.log("DEBUG processAudioBlobAndInsertText: Ejecutando lógica de NUEVO DICT(`Bloqueado:${d.promptFeedback.blockReason}.${d.promptFeedback.blockReasonMessage||''}`);if(d.candidates?.[0]?.finishReason&&d.candidates[0].finishReason!=="STOP")throw new Error(`Gemini fin:${d.candidates[0].finishReason}.`);if(d.candidates?.[0]?.finishReason==="STOP"&&!d.candidates?.[0]?.content?.parts?.[0]?.text)return"";throw new Error('Gemini respuesta inesperada.');}
...
async function transcribeAndPolishAudio(b){
    let tTxt='';
    try{
        setStatus('Transcribiendo...','processing');
        const tP=[{text:"Transcribe el audio a texto LITERALMENTE. No corrijas. Si dice 'coma', 'punto', transcribe 'coma', 'punto'."},{inline_data:{mime_type:"audio/webm",data:b}}];
        tTxt=await callGeminiAPI(tP,false);
    ...
    if(!tTxt||tTxt.trim()==="") throw new Error("Transcripción vacía.");
    
    let pAI='';
    try{
        setStatus('Puliendo...','processing');
        const pP=[{text:`Por favor, revisa el siguiente texto y aplica ÚNICAMENTEADO (reemplazar todo).");
            if (processedNewText.length > 0) { 
                // Para un dictado nuevo, el primer carácter siempre debe ser mayúscula.
                processedNewText = processedNewText.charAt(0).toUpperCase() + processedNewText.slice(1);
            }
            console.log("DEBUG processAudioBlobAndInsertText (Nuevo Dictado): Texto procesado con capitalización inicial:", JSON.stringify(processedNewText.substring(0,100) + "..."));
            polishedTextarea.value = processedNewText; 
...
function updateButtonStates(state) { startRecordBtn.disabled=true;pauseResumeBtn.disabled=true;retryProcessBtn.disabled=true;copyPolishedTextBtn.disabled=false;correctTextSelectionBtn.disabled=true;startRecordBtn.textContent="Empezar Dictado";startRecordBtn.classList.remove("stop-style");pauseResumeBtn.textContent="Pausar";let showPlayer=false;if(currentAudioBlob){if([" las siguientes modificaciones:\n1. Interpreta y reemplaza las siguientes palabras dictadas como signos de puntuación y formato EXACTAMENTE como se indica:\n    * 'coma' -> ','\n    * 'punto' -> '.'\n    * 'punto y aparte' -> '.' seguido de UN ÚNICO SALTO DE LÍNEA (\\n).\n    * 'nueva línea' o 'siguiente línea' -> un único salto de línea (\\n).\n    * 'dos puntos' -> ':'\n    * 'punto y coma' -> ';'\n    * 'signo de interrogación', 'interrogación' o 'pregunta' (al final de una frase) -> '?'\n    * 'signo de exclamación', 'exclamación' o 'admiración' (al final de una frase) -> '!'\n2. Corrige ÚNICAMENTE errores ortográficos evidentes.\n3. Corrige ÚNICAMENTE errores gramaticales OBJETIVOS Y CLAROS.\n4. NO CAMBIES la elección de palabras del hablante si son gramaticalmente correctas.\n5. NO REESTRUCTURES frases si son gramaticalmente correctas.\n6. PRESERVA el estilo y las expresiones exactas del hablante.\n7. Si el texto ya contiene puntuación, no la dupliques.\n8. IMPORTANTE: NO añadas un punto final (.) u otra puntuación final al texto a menos que se haya dictado explícitamente la palabra "punto", "pregunta", "exclamación", o similar. Si el dictado termina sin una instrucción de puntuación, devuelve el texto tal cual termina.\n\nTexto a procesar:\n"${tTxt}"`}];
    ...
    console.log("DEBUG transcribeAndPolishAudio: Texto FINAL (antes;correctTextSelectionBtn.disabled=true;break;case "processing_audio":startRecordBtn.disabled=true;pauseResumeBtn.disabled=true;retryProcessBtn.disabled=true;correctTextSelectionBtn.disabled=true;break;case "error_processing":startRecordBtn.disabled=false;retryProcessBtn.disabled=!currentAudioBlob;correctTextSelectionBtn.disabled=polishedTextarea.value.trim()==="";break;case "success_processing":startRecordBtn.disabled=false;retryProcessBtn.disabled=!currentAudioBlob;correctTextSelectionBtn.disabled=polishedTextarea.value.trim()==="";break;case "error":startRecordBtn.disabled=false;resetRecordingTimerDisplay();stopVolumeMeter();retryProcessBtn.disabled=!currentAudioBlob;correctTextSelectionBtn.disabled=true;break;default:startRecordBtn.disabled=false;resetRecordingTimerDisplay();stopVolumeMeter();retryProcessBtn.disabled=!currentAudioBlob;correctTextSelectionBtn.disabled=polishedTextarea.value.trim()==="";break;}}
async function handleCorrectTextSelection(){if(!polishedTextarea)return;const sS=polishedTextarea.selectionStart;const sE=polishedTextarea.selectionEnd;const sT=polishedTextarea.value.substring(sS,sE).trim();if(!sT){setStatus("Selecciona texto.","idle",3000);return;}const cTU=prompt(`Corregir:\n"${sT}"\n\nCorrección:` ,sT);if(cTU===null){setStatus("Cancelado.","idle",2000);return;}const fCT=cTU.trim();const ruleKey = sT.toLowerCase(); if(sT.toLowerCase()===fCT.toLowerCase()&&sT!==fCT){}else if(sT.toLowerCase()!==fCT.toLowerCase()||fCT== de capitalización contextual):", JSON.stringify(finT.substring(0,150) + "..."));
    return finT;
}
...
async function handleAddNewVocabRule() { const errorKeyRaw = prompt("Texto incorrecto (o palabra a reemplazar):"); if (!errorKeyRaw || errorKeyRaw.trim() === "") return; const errorKey = errorKeyRaw.trim().toLowerCase(); const correctValueRaw = prompt(`Corrección para "${errorKeyRaw}":`);(`Bloqueado:${d.promptFeedback.blockReason}.${d.promptFeedback.blockReasonMessage||''}`);if(d.candidates?.[0]?.finishReason&&d.candidates[0].finishReason!=="STOP")throw new Error(`Gemini fin:${d.candidates[0].finishReason}.`);if(d.candidates?.[0]?.finishReason==="STOP"&&!d.candidates?.[0]?.content?.parts?.[0]?.text)return"";throw new Error('Gemini respuesta inesperada.');}
...
function capitalizeSentencesProperly(text) {
    ...
    let processedText = text.trim(); 
    processedText = processedText.replace(
        /([.!?])(\s*)([a-záéíóúüñ])/g, // Busca PUNTUACIÓN + ESPACIOS + LETRA MINÚSCULA
        (match, punctuation, whitespace, letter) => {
            return punctuation + whitespace + letter.toUpperCase();
        }
    );
    // No se capitaliza el primer carácter del fragmento aquí, se hace en processAudioBlobAndInsertText
    return processedText;
}
