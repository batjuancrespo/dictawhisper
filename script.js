...
function initializeAuthAndApp() {
    ...
    onAuthStateChanged(auth, (user) => {
        ...
        if (user) {
            ...
            authContainer.style.display = 'none'; appContainer.style.display = 'flex'; /* app-container es flex */
...
function initializeDictationAppLogic(userId) {
    ...
    if (!techniqueButtonsContainer.dataset.listenerAttached) { techniqueButtonsContainer.addEventListener('click', (e) => { if (e.target.tagName === 'BUTTON' && e.target.dataset.techniqueText) { headerArea.value = e.target.dataset.techniqueText; headerArea.focus(); }}); techniqueButtonsContainer.dataset.listenerAttached = 'true';}
    if (!clearHeaderButton.dataset.listenerAttached) { clearHeaderButton.addEventListener('click', () => { headerArea.value = ""; headerArea.focus(); }); clearHeaderButton.dataset.listenerAttached = 'true';}
