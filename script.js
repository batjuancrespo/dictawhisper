...
let startRecordBtn, pauseResumeBtn, retryProcessBtn, copyPolishedTextBtn, correctTextSelectionBtn,
    ...
    vocabManagerModal, vocabManagerList, modalCloseButtonVocab, modalAddNewRuleButtonVocab, manageVocabButton; // Para el modal
...
function initializeAuthAndApp() {
    ...
    onAuthStateChanged(auth, async (user) => { 
        ...
        if (user) {
            ...
            currentUserId = null; currentUserVocabulary = {}; 
...
// --- FUNCIONES DE GESTIÓN DE VOCABULARIO Y MODAL ---
async function loadUserVocabulary(userId) { if (!userId || !window.db) { currentUserVocabulary = {}; return; } const vocabDocRef = window.doc(window.db, "userVocabularies", userId); try { const docSnap = await window.getDoc(vocabDocRef); if (docSnap.exists()) { currentUserVocabulary = docSnap.data().rules || {}; } else { currentUserVocabulary = {}; } console.log("DEBUG: Vocabulario cargado:", Object.keys(currentUserVocabulary).length, "reglas"); } catch (error) { console.error("Error cargando vocabulario:", error); currentUserVocabulary = {}; } }
async function saveUserVocabulary() { if (!currentUserId || !window.db) { console.error("DEBUG: No hay userId o DB para guardar vocabulario."); return; } const vocabDocRef = window.doc(window.db, "userVocabularies", currentUserId); try { await window.setDoc(vocabDocRef, { rules: currentUserVocabulary }, { merge: true }); console.log("DEBUG: Vocabulario del usuario guardado."); } catch (error) { console.error("Error guardando vocabulario:", error); setStatus("Error al guardar personalizaciones.", "error", 3000); } }
function applyUserVocabulary(text) { if (!text || Object.keys(currentUserVocabulary).length === 0) return text; let processedText = text; const sortedKeys = Object.keys(currentUserVocabulary).sort((a, b) => b.length - a.length); for (const errorKey of sortedKeys) { const correctValue = currentUserVocabulary[errorKey]; try { const regex = new RegExp(`\\b${escapeRegExp(errorKey)}\\b`, 'gi'); processedText = processedText.replace(regex, correctValue); } catch (e) { console.error(`Error regex para: "${errorKey}"`, e); } } return processedText; }
