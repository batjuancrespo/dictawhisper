...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateTimer() {
    ...
    timerElement.textContent = `Tiempo de grabación: ${minutes}:${seconds.toString().padStart(2, '0')}`;
  }
  ...
  async function processAndTranscribeChunks(existingText = '') {
    const totalAudioBlob = new Blob(audioChunks, { type: 'audio/wav' });
  ...
  async function transcribeChunk(audioBlob) {
    ...
    const originalTranscribeChunk = async (audioBlob) => {
      loadingElement.classList.remove('hidden');
      statusElement.textContent = `Transcribiendo fragmento de audio ${chunkCount + 1}...`;
      chunkCount++;
      ...
      try {
        ...
        console.log(`=== TRANSCRIPCIÓN CHUNK ${chunkCount} ===`);
        console.log("Texto raw:", rawText);

        return rawText;

      } catch (error) {
        console.error(`Error en la transcripción del chunk ${chunkCount}:`, error);
    ...
    let text = await originalTranscribeChunk(audioBlob);
