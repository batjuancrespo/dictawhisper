...
let currentUserVocabulary = {}; // Almacenará el 'rulesMap'
let learnedCorrectionsMap = {}; // Para 'learnedMap'
let commonMistakeNormalizationMap = {}; // Para 'normalizations'
...
function initializeAuthAndApp() {
    ...
    onAuthStateChanged(auth, async (user) => { 
        ...
        if (user) {
            ...
            await loadUserVocabulary(currentUserId); 
            ...
            currentUserId = null; currentUserVocabulary = {}; learnedCorrectionsMap = {}; commonMistakeNormalizationMap = {};
...
async function handleCorrectTextSelection(){if(!polishedTextarea)return;const sS=polishedTextarea.selectionStart;const sE=polishedTextarea.selectionEnd;const sT=polishedTextarea.value.substring(sS,sE).trim();if(!sT){setStatus("Selecciona texto.","idle",3000);return;}const cTU=prompt(`Corregir:\n"${sT}"\n\nCorrección:` ,sT);if(cTU===null){setStatus("Cancelado.","idle",2000);return;}const fCT=cTU.trim();if(sT.toLowerCase()===fCT.toLowerCase()&&sT!==fCT){}else if(sT.toLowerCase()!==fCT.toLowerCase()||fCT===""){currentUserVocabulary[sT.toLowerCase()]=fCT;await saveUserVocabulary();setStatus(`Regla guardada: "${sT.toLowerCase()}"➔"${fCT}"`,"success",3000);}else{setStatus("No cambios.","idle",2000);}const tB=polishedTextarea.value.substring(0,sS);const tA=polishedTextarea.value.substring(sE);polishedTextarea.value=tB+fCT+tA;polishedTextarea.selectionStart=polishedTextarea.selectionEnd=sS+fCT.length;polishedTextarea.focus();}
...
async function transcribeAndPolishAudio(b){let tTxt='';try{setStatus('Transcribiendo...','processing');const tP=[{text:"Transcribe el audio a texto LITERALMENTE. No corrijas. Si dice 'coma', 'punto', transcribe 'coma', 'punto'."},{inline_data:{mime_type:"audio/webm",data:b}}];tTxt=await callGeminiAPI(tP,false);console.log("---Transcripción Original (Consola)---\n",tTxt,"\n-----------------------------------");}catch(e){console.error("Error transcripción:",e);throw new Error(`Fallo transcripción:${e.message}`);}if(!tTxt||tTxt.trim()==="")throw new Error("Transcripción vacía.");try{setStatus('Puliendo...','processing');const pP=[{text:`Revisa y pule. INSTRUCCIONES:\n1.Interpreta palabras dictadas como signos:\n'coma'➔, 'punto'➔. 'punto y aparte'➔.(salto línea único) 'nueva línea'➔(salto línea único) 'dos puntos'➔: 'punto y coma'➔; 'interrogación'➔? 'exclamación'➔!\n2.Corrige SOLO ortografía/gramática OBVIA.\n3.NO CAMBIES palabras/estructura si es OK.\n4.PRESERVA estilo.\n5.Si ya OK, cambios MÍNIMOS.\n6.Capitaliza inicio de frases.\n\nTexto:"${tTxt}"`}];let pR=await callGeminiAPI(pP,true);let cT=capitalizeSentencesProperly(pR);let postT=applyUserVocabulary(cT);postT=postT.replace(/\.\s*\n\s*\n/g,'.\n').replace(/\n\s*\n/g,'\n');return postT;}catch(e){console.error("Error pulido:",e);setStatus(`Fallo pulido:${e.message}. Usando cruda.`,"error",4000);let cF=capitalizeSentencesProperly(tTxt);return applyUserVocabulary(cF);}}
async function loadUserVocabulary(userId) { if (!userId || !window.db) { currentUserVocabulary = {}; learnedCorrectionsMap = {}; commonMistakeNormalizationMap = {}; return; } console.log(`DEBUG: Cargando vocabulario (formato completo) para usuario: ${userId}`); const vocabDocRef = window.doc(window.db, "userVocabularies", userId); try { const docSnap = await window.getDoc(vocabDocRef); if (docSnap.exists()) { const firestoreData = docSnap.data(); currentUserVocabulary = firestoreData.rulesMap || {}; learnedCorrectionsMap = firestoreData.learnedMap || {}; commonMistakeNormalizationMap = firestoreData.normalizations || {}; console.log("DEBUG: Vocabulario cargado desde Firestore. Reglas directas:", Object.keys(currentUserVocabulary).length, "Correcciones aprendidas:", Object.keys(learnedCorrectionsMap).length, "Normalizaciones:", Object.keys(commonMistakeNormalizationMap).length ); } else { console.log("DEBUG: No existe documento de vocabulario. Usando vocabularios vacíos."); currentUserVocabulary = {}; learnedCorrectionsMap = {}; commonMistakeNormalizationMap = {}; } } catch (error) { console.error("Error cargando vocabulario del usuario desde Firestore:", error); setStatus("Error al cargar personalizaciones.", "error", 3000); currentUserVocabulary = {}; learnedCorrectionsMap = {}; commonMistakeNormalizationMap = {}; } }
async function saveUserVocabulary() { if (!currentUserId || !window.db) { console.error("DEBUG: saveUserVocabulary - No hay userId o DB."); return; } console.log(`DEBUG: Guardando vocabulario para usuario: ${currentUserId}.`); const vocabDocRef = window.doc(window.db, "userVocabularies", currentUserId); const dataToSave = { rulesMap: currentUserVocabulary, learnedMap: learnedCorrectionsMap, normalizations: commonMistakeNormalizationMap }; console.log("DEBUG: Datos a guardar en Firestore:", dataToSave); try { await window.setDoc(vocabDocRef, dataToSave, { merge: false }); console.log("DEBUG: Vocabulario del usuario guardado en Firestore (formato completo)."); } catch (error) { console.error("Error guardando vocabulario del usuario:", error); setStatus("Error al guardar personalizaciones.", "error", 3000); } }
function applyUserVocabulary(text) { if (!text || Object.keys(currentUserVocabulary).length === 0) return text; let processedText = text; console.log("DEBUG: Aplicando currentUserVocabulary (rulesMap). Reglas:", Object.keys(currentUserVocabulary).length); const sortedKeys = Object.keys(currentUserVocabulary).sort((a, b) => b.length - a.length); for (const errorKey of sortedKeys) { const correctValue = currentUserVocabulary[errorKey]; try { const regex = new RegExp(`\\b${escapeRegExp(errorKey)}\\b`, 'gi'); processedText = processedText.replace(regex, correctValue); } catch (e) { console.error(`Error regex para (rulesMap): "${errorKey}"`, e); } } return processedText; }
...
function populateVocabManagerList() { vocabManagerList = vocabManagerList || document.getElementById('vocabManagerList'); if (!vocabManagerList) { console.error("Lista del modal de vocabulario no encontrada."); return; } vocabManagerList.innerHTML = ''; const नियमKeys = Object.keys(currentUserVocabulary).sort(); if (नियमKeys.length === 0) { vocabManagerList.innerHTML = '<li>No hay reglas personalizadas.</li>'; return; } नियमKeys.forEach(key => { const value = currentUserVocabulary[key]; const listItem = document.createElement('li'); listItem.innerHTML = `<span class="vocab-key">${key}</span> <span class="vocab-arrow">➔</span> <span class="vocab-value">${value}</span> <div class="vocab-actions"><button class="edit-vocab-btn" data-key="${key}">Editar</button><button class="delete-vocab-btn" data-key="${key}">Borrar</button></div>`; listItem.querySelector('.edit-vocab-btn').addEventListener('click', () => handleEditVocabRule(key)); listItem.querySelector('.delete-vocab-btn').addEventListener('click', () => handleDeleteVocabRule(key)); vocabManagerList.appendChild(listItem); }); }
async function handleAddNewVocabRule() { const errorKeyRaw = prompt("Texto incorrecto (o palabra a reemplazar):"); if (!errorKeyRaw || errorKeyRaw.trim() === "") return; const errorKey = errorKeyRaw.trim().toLowerCase(); const correctValueRaw = prompt(`Corrección para "${errorKeyRaw}":`); if (correctValueRaw === null) return; const correctValue = correctValueRaw.trim(); if (currentUserVocabulary[errorKey] === correctValue && correctValue !== "") { alert("Regla ya existe con el mismo valor."); return; } currentUserVocabulary[errorKey] = correctValue; await saveUserVocabulary(); populateVocabManagerList(); setStatus("Regla añadida/actualizada.", "success", 2000); }
async function handleEditVocabRule(keyToEdit) { const currentValue = currentUserVocabulary[keyToEdit]; const newErrorKeyRaw = prompt(`Editar CLAVE (original: "${keyToEdit}"):\n(Dejar vacío para mantener la clave original)`, keyToEdit); if (newErrorKeyRaw === null) return; const newErrorKey = (newErrorKeyRaw.trim() === "" ? keyToEdit : newErrorKeyRaw.trim()).toLowerCase(); const newCorrectValueRaw = prompt(`Editar VALOR para "${newErrorKey}" (original: "${currentValue}"):`, currentValue); if (newCorrectValueRaw === null) return; const newCorrectValue = newCorrectValueRaw.trim(); if (newErrorKey !== keyToEdit && currentUserVocabulary.hasOwnProperty(newErrorKey)) { alert(`La clave "${newErrorKey}" ya existe.`); return; } if (newErrorKey !== keyToEdit) delete currentUserVocabulary[keyToEdit]; currentUserVocabulary[newErrorKey] = newCorrectValue; await saveUserVocabulary(); populateVocabManagerList(); setStatus("Regla actualizada.", "success", 2000); }
async function handleDeleteVocabRule(keyToDelete) { if (confirm(`¿Borrar la regla para "${keyToDelete}"?`)) { delete currentUserVocabulary[keyToDelete]; await saveUserVocabulary(); populateVocabManagerList(); setStatus("Regla borrada.", "success", 2000); } }
